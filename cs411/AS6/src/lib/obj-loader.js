...
function readOBJFile(fileName, gl, model, scale, reverse) {
  return new Promise((resolve, reject) => {
    model.arrays=null;
    var request = new XMLHttpRequest(); // retrieve data from URL without full page refresh (need not be XML)
  
    request.onreadystatechange = function() {
      if (request.readyState === 4 && request.status !== 404) {
        resolve(onReadOBJFile(request.responseText, fileName, gl, model, scale, reverse));
      }
      //reject(request);
    }
...
// Check if object file is fully loaded
function isOBJFileLoaded(model) {
  if (model.objDoc != null && model.objDoc.isMTLComplete()) return true;
  else return false;
}


// Extract object file arrays 
function extractOBJFileArrays(model) {
  if(isOBJFileLoaded(model)) {
    model.arrays = model.objDoc.getDrawingInfo();
    return true;
  }
  else return false;
}
...
var OBJDoc = function(fileName) {
  ...
  //this.textureCoords = new Array(0);   // Initialize the property textureCoordinates /////
}
...
OBJDoc.prototype.parseFace = function(sp, materialName, vertices, reverse) {  
  ...
  // handle failure
  if (normal == null) {
    if (face.vIndices.length >= 4) { // handle more than 3 vertices
      var v3 = [
  ...
  // divide to triangles if Fce contains over 3 points.
...
//      var textureCoord = face.textureCoord; ///////
      for(var k = 0; k < face.vIndices.length; k++){
        // Set index
        indices[index_indices] = index_indices;  //// here a new index is created for each vertex
                                                 //// regardless of it is already there
        // Copy vertex
        ...
        // Copy textureCoords
//        textureCoords[index_indices * 4 + 0] = textureCoord.x; ///////
//        textureCoords[index_indices * 4 + 1] = textureCoord.y;
        // Copy normal
...
// TextureCoord Object 
//------------------------------------------------------------------------------
//var TextureCoord = function(x, y, z) { ////////
//  this.x = x;
//  this.y = y;
//}
